...
/// GPU.
pub struct OpenCLNaiveEngine {
    program: Program,
    device: Device,
    context: Context,
}

impl OpenCLNaiveEngine {
    pub fn new(gpu_index: usize) -> Self {
        // The OpenCL program source code.
        let program_src: String = include_str!("opencl_naive.cl")
...
impl Engine for OpenCLNaiveEngine {
    fn name(&self) -> String {
        format!("OpenCL Naive (GPU: {})", self.device.name().unwrap())
    }
